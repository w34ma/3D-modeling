                 ...
                 const vect3f& local_y_axis, const vect3f& local_z_axis)
{
   // PUT YOUR SOLUTION HERE
    //center + z
    double scale = 0.06;
    point3f p[5] = {point3f(center_p.x_+ 2 * scale * local_z_axis.dx_,center_p.y_+ 2 * scale * local_z_axis.dy_,center_p.z_+ 2 * scale * local_z_axis.dz_),
        //center + 1/2x + 1/2y
        point3f(center_p.x_+ scale*local_x_axis.dx_ ,center_p.y_+ scale*local_x_axis.dy_ ,center_p.z_+ scale*local_x_axis.dz_),
        //center - 1/2x + 1/2y
        point3f(center_p.x_- scale*local_x_axis.dx_ ,center_p.y_- scale*local_x_axis.dy_ ,center_p.z_- scale*local_x_axis.dz_),
        //center - 1/2x - 1/2y
        point3f(center_p.x_+ scale*local_y_axis.dx_ ,center_p.y_+ scale*local_y_axis.dy_ ,center_p.z_+ scale*local_y_axis.dz_),
        //center - 1/2x + 1/2y
        point3f(center_p.x_- scale*local_y_axis.dx_ ,center_p.y_- scale*local_y_axis.dy_ ,center_p.z_- scale*local_y_axis.dz_)};
    ...
    for (int i = 1; i <= 4; i++)
    {
        for (int j = i + 1; j <= 4; j++)
        {
            glVertex3f(p[i].x_, p[i].y_, p[i].z_);
            glVertex3f(p[j].x_, p[j].y_, p[j].z_);
        }
        glVertex3f(p[0].x_, p[0].y_, p[0].z_);
        glVertex3f(p[i].x_, p[i].y_, p[i].z_);
    }
                      ...
                      const point3f& r0, const vect3f& r)
{
    ...
    //v= m x r ：u x v = { u2v3-v2u3 , u3v1-v3u1 , u1v2-u2v1 }
    ...
    vect3f N;
    N.dx_= r.dy_*V.dz_ - V.dy_*r.dz_;
    N.dy_= r.dz_*V.dx_ - V.dz_*r.dx_;
    N.dz_= r.dx_*V.dy_ - V.dx_*r.dy_;
            ...
            double rot[])
{
    ...
    vect3f v(old_pos.x_ - new_pos.x_, old_pos.y_ - new_pos.y_, 0); // not sure if the last one is 0?
    //A = v x n rotate axis A //v= m x r ：u x v = { u2v3-v2u3 , u3v1-v3u1 , u1v2-u2v1 }
    vect3f A;
    A.dx_= v.dy_*n.dz_ - n.dy_*v.dz_;
    A.dy_= v.dz_*n.dx_ - n.dz_*v.dx_;
    A.dz_= v.dx_*n.dy_ - n.dx_*v.dy_;
    
    glRotatef(1 * sqrt(A.dx_ * A.dx_ + A.dy_ * A.dy_ + A.dz_ * A.dz_), A.dx_, A.dy_, A.dz_);//k = 2?
               ...
               double translation[])
{
    ...
    point3f pcurr =closest_point(m1, M, view_p, Rcurr);
    point3f pstart =closest_point(m1, M, view_p, Rstart);
    glTranslatef(pcurr.x_ - pstart.x_, pcurr.y_ - pstart.y_, pcurr.z_ - pstart.z_);
 
    float mat[16];
    glGetFloatv(GL_MODELVIEW_MATRIX,mat);
	for(int i = 0; i< 16;i++){
		translation[i] = (double)mat[i];
	}
           ...
           double scaling[])
{
    ...
    float val1 =sqrt((pcurr.x_ - m1.x_) * (pcurr.x_ - m1.x_) + (pcurr.y_ - m1.y_) * (pcurr.y_ - m1.y_)+(pcurr.z_ - m1.z_) * (pcurr.z_ - m1.z_));
    float val2 =sqrt((pstart.x_ - m1.x_) * (pstart.x_ - m1.x_) + (pstart.y_ - m1.y_) * (pstart.y_ - m1.y_) + (pstart.z_ - m1.z_) * (pstart.z_ - m1.z_));
